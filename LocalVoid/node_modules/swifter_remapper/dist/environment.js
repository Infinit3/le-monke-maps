...
(function (BlenderEnvironmentInternals) {
        ...
        processData(trackData) {
            ...
            trackData.forEach(x => {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    let ref = new animation_1.Keyframe(longestArr[i]);
                    posIndex++;
                    rotIndex++;
                    scaleIndex++;
                    ...
                    let offset = (0, general_1.rotatePoint)(objRot, objScale.map((y, i) => y * -this.anchor[i] * blenderShrink));
    ...
    class BlenderAssigned extends BaseBlenderEnvironment {
        constructor(scale, anchor, track, disappearWhenAbsent) {
            super(scale, anchor);
            this.track = track;
        ...
        static(dataTrack, forEvents = undefined) {
            ...
            if (data.length > 0) {
                ...
                let moveEvent = new custom_event_1.CustomEvent().animateTrack(this.track);
        ...
        animate(dataTrack, time, duration, forEvents = undefined) {
            ...
            if (data.length > 0) {
                let x = data[0];
                let moveEvent = new custom_event_1.CustomEvent(time).animateTrack(this.track);
    ...
    */
    ...
    animate(switches, forEnvSpawn = undefined) {
        ...
        switches.forEach(x => {
            ...
            let dataTrack = x[0];
            let time = x[1];
            let duration = (_a = x[2]) !== null && _a !== void 0 ? _a : 0;
            let forEnv = x[3];
            let forAssigned = x[4];
            let data = this.processData(dataTrack);
            let objects = 0;
            data.forEach((x, i) => {
                let event = new custom_event_1.CustomEvent(time).animateTrack(this.getPieceTrack(i), duration);
        ...
        switches.forEach(x => {
            ...
            for (let i = objects; i < this.maxObjects; i++) {
                let event = new custom_event_1.CustomEvent(time).animateTrack(this.getPieceTrack(i));
...
function animateEnvGroup(group, time, duration, animation, easing = undefined) {
    if (beatmap_1.activeDiff.environment !== undefined)
        beatmap_1.activeDiff.environment.forEach(x => {
            if (x.group === group) {
                let newAnimation = (0, general_1.copy)(animation.json);
...
function getTrackData(track) {
    if (!trackData[track]) {
        ...
        for (let i = 0; i < beatmap_1.activeDiff.notes.length; i++) {
            let note = beatmap_1.activeDiff.notes[i];
