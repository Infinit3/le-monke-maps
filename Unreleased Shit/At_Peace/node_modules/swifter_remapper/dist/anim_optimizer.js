...
exports.optimizeAnimation = exports.OptimizeSettings = exports.OptimizeSimilarPointsSlopeSettings = exports.OptimizeSimilarPointsSettings = exports.OptimizeFloatingPointsSettings = void 0;
const animation_1 = require("./animation");
const general_1 = require("./general");
function areArrayElementsIdentical(enumerable1, enumerable2) {
...
function areFloatsSimilar(enumerable1, enumerable2, threshold) {
    if (enumerable1.length !== enumerable2.length) {
...
function arePointSimilar(a, b, differenceThreshold, timeDifferenceThreshold) {
...
function ComparePointsSlope(startPoint, middlePoint, endPoint, 
// pass in array to reuse and avoid allocations
middleSlope, endSlope, middleYIntercepts, endYIntercepts, timeDifferenceThreshold, differenceThreshold, yInterceptDifferenceThreshold) {
...
function GetYIntercept(pointData, slopeArray, yIntercepts) {
...
function SlopeOfPoint(a, b, slopes) {
...
function optimizeFloatingPoints(a, b, c, optimizeFloatingPoints) {
    [a, b, c].forEach((p) => {
        if (!p)
            return;
...
function optimizeDuplicates(pointA, pointB, pointC) {
    if (pointC === undefined) {
        // array is size 2
...
function optimizeSimilarPoints(pointA, pointB, pointC, settings) {
...
function optimizeSimilarPointsSlope(pointA, pointB, pointC, settings) {
    if (pointC === undefined) {
        // array is size 2
        return undefined;
    }
 ...
 */
class OptimizeFloatingPointsSettings {
    constructor() {
        this.decimals = 5;
    }
}
exports.OptimizeFloatingPointsSettings = OptimizeFloatingPointsSettings;
/**
 * Settings for the "optimizeSimilarPoints" optimizing function, starts at default values.
 */
class OptimizeSimilarPointsSettings {
    constructor() {
        ...
        this.timeDifferenceThreshold = 0.001;
    }
}
exports.OptimizeSimilarPointsSettings = OptimizeSimilarPointsSettings;
/**
...
class OptimizeSimilarPointsSlopeSettings {
    constructor() {
        ...
        // These numbers at quick glance seem to be fairly reliable, nice
        // however they should be configurable or looked at later
        this.differenceThreshold = 0.03;
        this.timeDifferenceThreshold = 0.025;
        this.yInterceptDifferenceThreshold = 0.5;
    }
}
exports.OptimizeSimilarPointsSlopeSettings = OptimizeSimilarPointsSlopeSettings;
/**
...
class OptimizeSettings {
    constructor() {
        ...
        this.passes = 5;
        this.performance_log = false;
        this.optimizeDuplicates = true; // false or undefined to disable
        this.optimizeFloatingPoints = new OptimizeFloatingPointsSettings();
        this.optimizeSimilarPoints = new OptimizeSimilarPointsSettings();
        this.optimizeSimilarPointsSlope = new OptimizeSimilarPointsSlopeSettings();
        this.additionalOptimizers = undefined;
    }
}
exports.OptimizeSettings = OptimizeSettings;
function optimizeKeyframes(keyframes, optimizeSettings) {
    var _a;
    const sortedKeyframes = keyframes.sort((a, b) => a.time - b.time);
    const optimizers = [...(_a = optimizeSettings.additionalOptimizers) !== null && _a !== void 0 ? _a : []];
    if (optimizeSettings.optimizeFloatingPoints)
        optimizers.push((a, b, c) => optimizeFloatingPoints(a, b, c, optimizeSettings.optimizeFloatingPoints));
    if (optimizeSettings.optimizeDuplicates)
        optimizers.push(optimizeDuplicates);
    if (optimizeSettings.optimizeSimilarPoints)
        optimizers.push((a, b, c) => optimizeSimilarPoints(a, b, c, optimizeSettings.optimizeSimilarPoints));
    if (optimizeSettings.optimizeSimilarPointsSlope)
        optimizers.push((a, b, c) => optimizeSimilarPointsSlope(a, b, c, optimizeSettings.optimizeSimilarPointsSlope));
 ...
 */
function optimizeAnimation(animation, settings) {
    const keyframes = (0, general_1.copy)((0, animation_1.complexifyArray)(animation)).map(x => new animation_1.Keyframe(x));
    // not enough points to optimize
    if (keyframes.length <= 2)
        return (0, animation_1.simplifyArray)(keyframes.map(x => x.data));
    return (0, animation_1.simplifyArray)(optimizeKeyframes(keyframes, settings).map(x => x.data));
}
exports.optimizeAnimation = optimizeAnimation;
