...
*/
function jsonGet(obj, prop, init) {
    // If the property doesn't exist, initialize it.
    if (init != null)
        jsonFill(obj, prop, init);
...
* @param {Any} value
*/
function jsonFill(obj, prop, value) {
    const steps = prop.split('.');
...
function worldToWall(pos, rot, scale) {
    ...
    const offset = rotatePoint(rot, scale.map((y, i) => y * wallOffset[i]));
...
function debugWall(pos = undefined, rot = undefined, scale = undefined, animStart = undefined, animDur = undefined, animFreq = 1 / 8) {
    ...
    const wall = new wall_1.Wall();
    wall.life = animDur + 69420;
    wall.lifeStart = 0;
    const wallAnim = new animation_1.Animation(wall.life).wallAnimation();
    const dataAnim = new animation_1.Animation().wallAnimation();
    ...
    function getDomain(arr) {
        ...
        let max = 0;
        arr.forEach(x => {
            const time = new animation_1.Keyframe(x).time;
            if (time < min)
                min = time;
            if (time > max)
                max = time;
    ...
    for (let i = totalMin; i <= totalMax; i += animFreq / animDur) {
        ...
        const objRot = dataAnim.get(constants_1.ANIM.ROTATION, i);
        const objScale = dataAnim.get(constants_1.ANIM.SCALE, i);
        objPos = worldToWall(objPos, objRot, objScale);
        data.pos.push([...objPos, time]);
        data.rot.push([...objRot, time]);
        data.scale.push([...objScale, time]);
    }
    wallAnim.add(constants_1.ANIM.DEFINITE_POSITION, data.pos);
    wallAnim.add(constants_1.ANIM.LOCAL_ROTATION, data.rot);
    wallAnim.add(constants_1.ANIM.SCALE, data.scale);
    wallAnim.optimize();
    wall.color = [0, 0, 0, 1];
